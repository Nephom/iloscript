...
func (c *ILOClient) Logout() {
        ...
        if len(sessionURL) > 0 && sessionURL[:len("/redfish/v1")] == "/redfish/v1" {
...
func (c *ILOClient) GetTaskStatus(taskURI string) (string, int, error) {
    ...
    if strings.HasPrefix(taskURI, "http") {
        url = taskURI
    } else if strings.HasPrefix(taskURI, "/") {
        url = c.BaseURL + taskURI
    } else {
        url = c.BaseURL + "/" + taskURI
    }
...
- SoftwareInventory
  - @odata.id: string
    */
    ...
    oemData, err := c.FetchOemHpeData()
    if err != nil {
        return "Error", 0, fmt.Errorf("failed to fetch OEM HPE data: %v", err)
    }
...
func main() {
	if len(os.Args) < 3 {
		...
		fmt.Println("Author: Nephom, Chiang")
		fmt.Println("Usage: <ilo_ip> <command> [<arguments>]")
		fmt.Println("Available Commands:")
		fmt.Println("  -model                   Show ProductName information.")
		fmt.Println("  -pca                     Show PCA information.")
		fmt.Println("  -ver                     Show Firmware information.")
		fmt.Println("  -devices                 Show PCI devices information.")
		fmt.Println("  -iml [fetch_count] [match_text] [severity_text]  Fetch IML events.")
		fmt.Println("  -iel                     Show EventLog containing 'Server'")
		fmt.Println("  -power [on|off]          Let SUT Power on or Off")
		fmt.Println("  -bios [get|setup|reset|patch] [attrName] [attrValue]   BIOS operation.")
		fmt.Println("  -sensors                 Get Thermal sensors value.")
		fmt.Println("  -reset                   CLear IML and AHS then set iLO to default value.")
		fmt.Println("  <firmware_url> [match_text]  Update firmware.")
		fmt.Println("Command Descriptions:")
		fmt.Println("  -model:")
		fmt.Println("    Fetch system ProductName")
		fmt.Println("  -pca:")
		fmt.Println("    Fetches and displays system info, including the serial number and iLO ASIC ID.")
		fmt.Println("    The iLO ASIC ID is then formatted as 'ilo_<asic_id>.bin'.")
		fmt.Println("  -reset:")
		fmt.Println("    CLear IML and AHS then set iLO to default value.")
		fmt.Println("  -iml:")
		fmt.Println("    Fetches the IML (Integrated Management Log) events.")
		fmt.Println("    Arguments:")
		fmt.Println("      fetch_count: Number of events to fetch (default: 10).")
		fmt.Println("      match_text: Filter events containing this text (default: empty).")
		fmt.Println("      severity_text: Filter severity containing this text (default: empty).")
		fmt.Println("    Example: -iml 20 SATA  (Fetch top 20 events containing 'SATA').")
		fmt.Println("             -iml SATA critical (Fetech top 10 events containing 'SATA' and Severity is 'Critical'")
		fmt.Println("    <firmware_url> [match_text]:")
		fmt.Println("    Updates the firmware from the provided URL and monitors the update status.")
		fmt.Println("    match_text (optional): Filter update logs based on this text.")
		fmt.Println("    Example: <firmware_url> - Update firmware from the provided URL.")
		fmt.Println("  -bios:")
		fmt.Println("    Arguments:")
		fmt.Println("      get:     Get All BIOS Settings")
		fmt.Println("      setup:   Get into BIOS Setup Menu when you power off.")
		fmt.Println("      reset:   Set BIOS to default setting when you power off and power on automatically")
		fmt.Println("      patch:   Set attrName and attrValue, please check on get arguments")
		fmt.Println("  -power on/off      Power on or power off the server")
		fmt.Println("  -ver               List all Firmware Version")
		fmt.Println("  -devices           List all PCI Devices")
		fmt.Println("  -sensors           List all Sensors")
		fmt.Println("Example Usages:")
		fmt.Println("  <ilo_ip> -pca                     # Show PCA info.")
		fmt.Println("  <ilo_ip> -iml 20 SATA             # Show top 20 events containing 'SATA'.")
		fmt.Println("  <ilo_ip> -power on                # Let iLO IP machine Power on!")
		fmt.Println("  <ilo_ip> <firmware_url> [match_text] # Update firmware and monitor status.")
		fmt.Println("  <ilo_ip> -bios patch AdminEmail test@sit.lab  # Update BIOS AdminEmail attribute to test@sit.lab")
		fmt.Println("Note: If you have issues with authentication, ensure the ILO_USERNAME and ILO_PASSWORD environment variables are set.")
		fmt.Println("        Default Account/password: Administrator / compaq")
		fmt.Println("")
	        os.Exit(0)
	}
	...
	case "-power":
		if len(os.Args) < 4 {
			fmt.Println("Usage: <ilo_ip> -power [on|off]")
			os.Exit(1)
		}
		powerAction := os.Args[3]
		if err := client.PowerControl(powerAction); err != nil {
	...
	case "-sensors":
		if err := client.FetchSensorData(); err != nil {
			fmt.Println("Sensor get failed: %v\n", err)
			os.Exit(1)
		}
	case "-iel":
